print("starting code")
import load
import model_architectures.model_linformer as model_sad
import numpy as np
import os
import argparse
import time
        ...
        for num_heads in [4, 8]:
            ...
            for k in [128, 256, 512, 1024]:
                for learning_rate in [0.001, 0.0001]: #[0.001, 0.0001, 0.00001]:
                    ...
                    sad_model = model_sad.SADModel(input_size, embed_size=hidden_size, num_heads=num_heads, num_layers=num_layers, seq_length=audio_size, k=k).to(device)
                    ...
                    best_val = 100
                    model_path = os.path.join(datadir_path, "models", f"model_linformer_{batch_size}-{audio_size}_{learning_rate}_{hidden_size}_{num_layers}.pt")
