...
for f_test in range(1):
    for batch_size, audio_size, overlap in [[10, 500, 100], [10, 200, 40], [10, 100, 20], [10, 50, 10], [10, 25, 5], [10, 10, 2], [10, 5, 1]]:
        ...
        for num_layers in [4]:#[2, 4]:
            for hidden_size in [[1024, 512]]: #[[512, 0.0001], [1024, 0.001], [1024, 0.0001]]:
                for learning_rate in [0.001, 0.0001]: #[0.001, 0.0001, 0.00001]:
                    ...
                    # model
                    sad_model = model_sad.SADModel(input_size=input_size, hidden_size=hidden_size, seq_lenght=audio_size, num_layers=None).to(device)
                    ...
                    model_path = os.path.join(datadir_path, "models", f"model_nn_{batch_size}-{audio_size}_{learning_rate}_{hidden_size}_{num_layers}.pt")
                    ...
                    if debug:
                        path = os.path.join(datadir_path, "plots_nn")
                    else:
                        path = "/storage/brno2/home/miapp/fearless-steps-SAD/fearless-steps-SAD/plots_nn"
