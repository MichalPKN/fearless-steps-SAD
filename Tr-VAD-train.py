print("starting code")
import load
import model_architectures.model_tr_vad as model_sad
import numpy as np
import os
import argparse
import time
        ...
        for num_layers in [2, 4]:
            for hidden_size in [128, 256, 512]:
                for learning_rate in [0.001, 0.0001, 0.00001]: #[0.001, 0.0001, 0.00001]:
                    ...
                    sad_model = model_sad.VADModel(dim_in=30, d_model=56, units_in=8, units=16, P=16, layers=2, drop_rate=0.3).to(device)
                    ...
                    best_val = 100
                    model_path = os.path.join(datadir_path, "models", f"model_tr_vad_{batch_size}-{audio_size}_{learning_rate}_{hidden_size}_{num_layers}.pt")
